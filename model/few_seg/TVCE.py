...
from model.few_seg.clip.clip_text import class_names, new_class_names, new_class_names_coco,isaid_class_names#, imagenet_templates
import argparse
...
def get_img_cam(que_img, tmp_que_name, que_class, model, bg_text_features, fg_text_features, cam, annotation_root,OT,flag = None):
# def get_img_cam(que_img, tmp_que_name, que_class, model, bg_text_features, fg_text_features, cam, annotation_root,Zero_Parameter):
    model = model.cuda()
    ...
    for i in range(0, len(tmp_que_name)):
        ...
        # print("Processing image:", que_name)
        if 'VOC' in annotation_root:
            ...
            if flag == False:
                tmp_label_img_path = os.path.join(annotation_root, 'val')
                label_img_path = os.path.join(tmp_label_img_path, que_name).replace('.png', '_instance_color_RGB.png')
            else:
                tmp_label_img_path = os.path.join(annotation_root, 'train')
                label_img_path = os.path.join(tmp_label_img_path, que_name).replace('.png', '_instance_color_RGB.png')
            ...
            label_id_list = [x  for x in label_id_list]

            label_list = []
            for lid in label_id_list:
                label_list.append(isaid_class_names[int(lid)])
            
        if que_class[i] not in label_id_list:
            return [torch.full((64,64),255).float().cuda()]
        # print("Calling img_ms_and_flip for image:", que_name)
        ...
        image = image.cuda()
        image_features, attn_weight_list,x1 = model.encode_image(image, h, w)
        x_no_pos = x1[:, :1024, :]
        ...
        text_features_temp1 = text_features_temp.clone()
        text_features_temp1 = text_features_temp1.unsqueeze(0) 
        
        out = OT(text_features_temp1,x_no_pos)
        out = out.squeeze(0)
        text_features_temp = text_features_temp + 0.001 * out
        input_tensor = [image_features, text_features_temp, h, w]
        ...
        for idx, label in enumerate(label_list):
            if 'VOC' in annotation_root:
                ...
                label_id = isaid_class_names.index(label)
            if label_id == que_class[i]:
                ...
                attn_weight = torch.mean(attn_weight, dim=0)###平均值  平均池化操作
