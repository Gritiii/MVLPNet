...
class Compose(object):
    ...
    def __call__(self, img_cv2, label1):
        for t in self.segtransform:
            img_cv2, label1 = t(img_cv2, label1)
        return img_cv2, label1
...
class ToTensor(object):
    ...
    def __call__(self, img_cv2, label1):
        if not isinstance(img_cv2, np.ndarray) or not isinstance(label1, np.ndarray):
            raise (RuntimeError("segtransform.ToTensor() only handle np.ndarray"
                                "[eg: data readed by cv2.imread()].\n"))
        if len(img_cv2.shape) > 3 or len(img_cv2.shape) < 2:
            raise (RuntimeError("segtransform.ToTensor() only handle np.ndarray with 3 dims or 2 dims.\n"))
        if len(img_cv2.shape) == 2:
            img_cv2 = np.expand_dims(img_cv2, axis=2)
        if not len(label1.shape) == 2:
            raise (RuntimeError("segtransform.ToTensor() only handle np.ndarray labellabel with 2 dims.\n"))

        img_cv2 = torch.from_numpy(img_cv2.transpose((2, 0, 1)))
        if not isinstance(img_cv2, torch.FloatTensor):
            img_cv2 = img_cv2.float()
...
class Normalize(object):
    ...
    def __call__(self, image, label, label2, img_cv2):
        if self.std is None:
            ...
            for t, m, s in zip(image, self.mean, self.std):
                t.sub_(m).div_(s)
        return image, label, label2, img_cv2
...
class Resize(object):
    ...
    def __call__(self, img_cv2, label1):
        ...
        img_cv2 = cv2.resize(img_cv2, dsize=self.size, interpolation=cv2.INTER_LINEAR)

        label1 = cv2.resize(label1.astype(np.float32), dsize=self.size, interpolation=cv2.INTER_NEAREST)
...
class RandScale(object):
    ...
    def __call__(self, img_cv2, label1):
        ...
        img_cv2 = cv2.resize(img_cv2, None, fx=scale_factor_x, fy=scale_factor_y, interpolation=cv2.INTER_LINEAR)
        label1 = cv2.resize(label1, None, fx=scale_factor_x, fy=scale_factor_y, interpolation=cv2.INTER_NEAREST)
        return img_cv2, label1
...
class RandRotate(object):
    ...
    def __call__(self, img_cv2, label1):
        if random.random() < self.p:
            ...
            img_cv2 = cv2.warpAffine(img_cv2, matrix, (w, h), flags=cv2.INTER_LINEAR, borderMode=cv2.BORDER_CONSTANT,
                                   borderValue=self.padding)

            label1 = cv2.warpAffine(label1, matrix, (w, h), flags=cv2.INTER_NEAREST, borderMode=cv2.BORDER_CONSTANT,
...
class RandomGaussianBlur(object):
    ...
    def __call__(self, img_cv2, label1):
        if random.random() < 0.5:
            img_cv2 = cv2.GaussianBlur(img_cv2, (self.radius, self.radius), 0)
        return img_cv2, label1
