...
from util1.util import AverageMeter,  intersectionAndUnionGPU, get_model_para_number, setup_seed, get_logger, get_save_path, \
...
# def train(train_loader, val_loader, model, optimizer, optimizer_swin, epoch ,scaler):
def train(train_loader, val_loader, model, optimizer, epoch ,scaler):
    ...
    for i, (input, input_name,target, s_input, s_mask,class_name,subcls,img_cv2) in enumerate(train_loader):
    # for i, (input, target, s_input, s_mask, subcls) in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        output, main_loss, aux_loss_1,aux_loss_2 = model(s_x=s_input, que_name=input_name,s_y=s_mask, x=input, x_cv2=img_cv2,y=target, cat_idx=subcls, class_name=class_name)
        ...
        # loss = main_loss + aux_loss_1 * args.aux_weight1 
        loss.backward()
        optimizer.step()
