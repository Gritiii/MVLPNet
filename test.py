...
from model.few_seg import MVLPNet
# from model.workdir import
from dataset import iSAID, LoveDA
from vis import Visualizer
from util1 import config,transform2
from util1.util import AverageMeter, poly_learning_rate, intersectionAndUnionGPU, get_model_para_number, setup_seed, get_logger, get_save_path, \
...
def get_parser():
    ...
    parser.add_argument('--arch', type=str, default='CMSANet', help='') # 
    parser.add_argument('--shot', type=int, default=1, help='') # 
    parser.add_argument('--split', type=int, default=2, help='') # 
    parser.add_argument('--dataset', type=str, default='iSAID', help='') # 
    parser.add_argument('--backbone', type=str, default='resnet50', help='') # 
    parser.add_argument('--visualize', action='store_true', default=True)
    parser.add_argument('--s_q', default='False', help='False') #
    parser.add_argument('--cross_domain', default='iSAID', help='') #
    ...
    print(base_config, data_config, model_config)
...
def get_model(args):
    ...
    if os.path.isfile(weight_path):
        ...
        if 'clip_model.visual.positional_embedding_new' in new_param:
            del new_param['clip_model.visual.positional_embedding_new']
        try: 
...
def main():
    ...
    train_data = eval('{}.{}_few_dataset'.format(args.dataset, args.dataset))(split=args.split, \
                                                shot=args.shot, mode='train', transform_dict=args.train_transform,transform_tri=train_transform_tri)
    train_id = []
    ...
    val_data =  eval('{}.{}_few_dataset'.format(args.dataset, args.dataset))(split=args.split, \
                                            shot=args.shot, mode='val', transform_dict=args.val_transform,transform_tri=val_transform_tri)
    val_data.list = []
    ...
    val_sampler = DistributedSampler(val_data,shuffle=False) if args.distributed else None                  
...
def validate(val_loader, model, val_seed):
    ...
    for e in range(10):
        ...
        for i, (input, input_name,target, s_input, s_mask, subcls, class_name,ori_label,img_cv2) in enumerate(val_loader):
            ...
            if eval(args.s_q):
                assert (args.shot==1)
                output = model(s_x=input.unsqueeze(1), que_name=input_name,s_y=s_mask.unsqueeze(1), x=input,x_cv2 = img_cv2, y=target, cat_idx=subcls,class_name = class_name)
                # with torch.no_grad():
                #     output = model(s_x=input.unsqueeze(1), s_y=target.unsqueeze(1), x=input, y=target, cat_idx=subcls)
            else:
                output = model(s_x=s_input, que_name=input_name,s_y=s_mask, x=input,x_cv2 = img_cv2, y=target, cat_idx=subcls,class_name = class_name)
                # with torch.no_grad():
